...
#>
param(
  [string] $ServerUrl = 'http://Aristotle:14145',                                  # Set to your Clone server URL
  [string] $MachineName = 'Aristotle',                                             # The machine name of the SQL Server instance to create the clones on
  [string] $InstanceName = '',                                                     # The instance name of the SQL Server instance to create the clones on
  [string] $ImageLocation = '\\Aristotle\SQLCloneImages',                          # Point to the file share we want to use to store the image
  [string] $MaskingSetPath = "\\Aristotle\MaskingSets\dmdemo_prodmask.DMSMaskSet", # The path to a masking set
  [string] $DatabaseName = 'DMDemo_5_Prod'                                         # The name of the database
)
...
# Remove the New Image if it exists for some reason
$ImageList = Get-SqlCloneImage
foreach ($Image in $ImageList) {
    if ($Image.Name -eq $ImageName) { Remove-SqlCloneImage $Image}
}
...
# TODO Error handling for image creation
